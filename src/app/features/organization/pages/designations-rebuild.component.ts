    ...
    <main class="h-full px-4 py-8 sm:px-6 lg:px-8">
      ...
      >
        ...
        <ui-badge org-list-status variant="primary" size="sm" [rounded]="true">
          Leveled Designations {{ leveledDesignationCount() }}
        </ui-badge>
        ...
        <section org-list-table-content class="overflow-hidden rounded-2xl border border-stone-200 bg-white shadow-sm dark:border-white/8 dark:bg-white/[0.04]">
          <app-organization-table-metadata itemLabel="Designations" [count]="designations().length" [lastRefreshedAt]="lastRefreshedAt()" />
          <div class="overflow-x-auto">
          <table class="min-w-full text-left">
            <thead app-organization-table-header-row [columns]="designationColumns"></thead>
            <tbody>
...
export class DesignationsRebuildComponent implements OnInit {
  ...
  readonly designationColumns: OrganizationTableColumn[] = [
    { label: 'Designation' },
    { label: 'Code' },
    { label: 'Level' },
    { label: 'Description' },
  ...
  leveledDesignationCount(): number {
    return this.designations().filter((designation) => typeof designation.level === 'number' && designation.level > 0).length;
  }

  unleveledDesignationCount(): number {
    return this.designations().length - this.leveledDesignationCount();
  }
